...
La caractérisation des [composantes valorisées](#cvportrait) a quant à elle été effectuée à partir de l'intégration de `r src(cv)` bases de données et résulté en la description de `r length(unique(cv$title))` stresseurs divisés en `r nrow(cv)` sous-catégories : `r ct2(cv)`. 
Enfin, des approches XXX ont été utilisées afin de caractériser la vulnérabilité relative de l'ensemble des catégories de composantes valorisées à l'ensemble des catégories des stresseurs environnementaux.

> *Mettre plus d'informations ici*

Nous présentons ici les constats généraux issus de l'évaluation des effets cumulatifs en points de forme. Des constats additionnels pourraient être faits en explorant davantage les résultats obtenus et nous invitons le lecteur à s'attarder à la section \@ref(resultats) du rapport pour davantage d'informations et à explorer les figures qui y sont présentées plus en détails.
...
> *Mettre plus d'informations ici, peut-être minimalement un lien vers les perspectives du rapport*
