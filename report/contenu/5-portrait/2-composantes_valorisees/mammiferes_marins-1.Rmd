...
Sur les neuf espèces dont une cartographie est disponible, cinq fréquentent l'estuaire du Saint-Laurent et le Saguenay (Tableau \@ref(tab:mmSp)). Pour notre étude, nous avons rééchantillonné les cartes de distribution produites [@wwfromm2021b] afin de les intégrer à notre grille d'étude de 1 $km^2$ en utilisant la valeur moyenne des celllules de la grille du guide qui intersecte chaque cellule de notre grille. Les valeurs ont ensuite été normalisées entre 0 et 1 en divisant chaque valeur par la valeur maximale observée au sein de la grille pour chaque espèce.
...
tab %>%
  knitr::kable(
    ...
    row.names = FALSE,
    caption = "Espèces de mammifères marins dont la distribution dans l'estuaire du Saint-Laurent et le Saguenay est disponible dans @wwfromm2021."
  )
```
