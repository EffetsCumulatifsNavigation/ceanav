#' Figure contribution régionale par km2 (english version)
#'
...
fig_regional_contribution_en <- function() {
...
gr2$gr2 <- gsub("Wolastoqiyik Wahsipekuk Nation","Wolastoqiyik\nWahsipekuk Nt.",gr2$gr2)
#=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=#
...
# Graph
#=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=#
png('./figures_en/figures-output/cumulative_effects_regional_contribution.png', res = 300, width = 600, height = 225, units = "mm")
...
# Text
text(x = -3, y = mean(c(0, dL[2])), labels = 'Mean stressor contribution to cumulative effects', srt = 90, adj = .5, font = 2)
...
for(i in 1:nrow(cekm)) {
  ...
  text(x = i-xG/4, y = yMax-.75, labels = cekm$english[i], cex = .75, srt = 90, adj = c(1,.5))
}
...
for(j in 1:nGroup) {
  ...
  for(i in 1:nrow(dat)) {
    ...
    text(x = x[j]+.2, y = dat$id[i], labels = dat$english[i], cex = .7, adj = c(0,.5))
  }
}
