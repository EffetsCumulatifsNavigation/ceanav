...
metadata_st_cv <- function() {
  ...
  for(i in st) {
    ...
    stresseurs[[i]] <- meta$dataDescription$categories %>%
                       ...
                       mutate(title = meta$title,
                              ...
                              stresseur = i) %>%
                       select(stresseur, accronyme, title, title_en, francais, description, source, english, description_en)
  }
  ...
  for(i in cv) {
    ...
    composantes_valorisees[[i]] <- meta$dataDescription$categories %>%
                       ...
                       mutate(title = meta$title,
                              title_en = meta$title_en,
                              comp_val = i) %>%
                       select(comp_val, accronyme, title, title_en, type, francais, source, english, type_en)#, description)
  }
  ...
  cv$simple <- gsub("Naturelle - Semi-végétalisée \\(IE = 1\\)", "Naturelle - Semi-végétalisée", cv$simple)
  cv$simple <- gsub("Naturelle - Vive \\(IE = 2\\)", "Naturelle - Vive", cv$simple)
  cv$simple <- gsub("Artificielle - Semi-végétalisée \\(IE = 1\\)", "Artificielle - Semi-végétalisée", cv$simple)
  cv$simple <- gsub("Artificielle - Vive \\(IE = 2\\)","Artificielle - Vive", cv$simple)
